export function initWrapper(wrapper) {
  ...
  wrapper.registerSolver = (nameOrSolver, mscFile, builtinDir, builtinFiles) => {
    let name = nameOrSolver;
    ...
    wrapper.on('ready', (instance) => {
      const fs = instance.fileSystem;
      try {
      ...
      try {
        ...
        Object.keys(builtinFiles || {}).forEach((key) => {
  ...
  const superCreateWorkerClient = wrapper.createWorkerClient;
  wrapper.createWorkerClient = function () {
    ...
    r.registerSolver = function (nameOrSolver, mscFile, builtinDir, builtinFiles) {
      ...
      return this.sendAndWaitOK({type: 'registerSolver', name, mscFile, builtinDir, builtinFiles});
    };
  ...
  const superCreateWorker = wrapper.createWorker;
