...
export class LodgingsDataHelper {
	...
	createLodgingsPaths(lodgings: any[]) {
		const lodgingsPaths = this.createLodgingsArrays(
			this.createLodgings(lodgings)
		);
	...
	createLodgingsArrays(lodgings: any[]) {
		...
		for (let i = 0; i < lodgings.length; i++) {
			...
			if (this.largestAmountOfFeatures > lodgings[i].features.length) {
				...
				for (
					let j = 0;
					j < this.largestAmountOfFeatures - featuresLength;
					j++
				) {
			...
			lodgingsPriceTotalAmount.push(Math.round(lodgings[i].price * 100));
		}
		return [
			lodgingsId,
			lodgingsStartDate,
